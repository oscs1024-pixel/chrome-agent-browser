...
多个 agent 会话可以同时连桥，每个会话有自己独立的受控标签页（连接级槽）：
新会话首次操作自动沿用最近打开的页面并在返回里提示，`tabs(select)` 选中
别人在操控的页面只警告不阻止。协议细节见 [`docs/协议.md`](docs/协议.md)。
...
6. **凭据隐去**——页面上成组出现的高熵字符串（恢复码、API key）会被替换成
   ...
   **这条是真实事故推出来的**：一次 `read_text` 曾把整页 2FA 恢复码读进对话上下文，
   而上下文是留痕的，进去了撤不回来。
7. **会话隔离**——受控 tab 按 agent 连接分槽、漂移基线按连接记录，
   并发 agent 的缺省调用不会再落到对方的页面上。

还没做完的，如实说：

| | 状态 |
|---|---|
| 站点白名单 | ⚠️ 代码在桥里，但 `allowlist.json` 不存在时**全放行**——也就是默认不生效 |
| 敏感动作弹窗二次确认 | ❌ 未实现。上面第 2 条是「不自动重试」，不是「问过你才做」 |

> **在这两条补齐之前，别接到网银和公司后台上。**
